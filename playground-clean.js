...
// Last updated: 2025-10-01 08:31:00 - Container positioning fixes applied
// CACHE REFRESH: Force browser to reload this file
class ArchitecturePlayground {
        ...
        if (palette) {
            palette.querySelectorAll('.palette-item').forEach(pi => {
                pi.addEventListener('dragstart', (e) => {
                    ...
                    console.log('Drag started with data:', dragData); // DEBUG: Log drag data
    ...
    setupCanvasDragAndDrop() {
        ...
        canvas.addEventListener('drop', (e) => {
            ...
            try {
                ...
                console.log('Drop data received:', data); // DEBUG: Log what data we're receiving
                const rect = canvas.getBoundingClientRect();
                ...
                // Determine item size based on type - containers are much larger
                const isContainer = data.type === 'canvas-item' && 
                    ['api-collection', 'schema-container', 'table-group', 'process-container', 'zone-container'].includes(data.itemType);
                const itemWidth = isContainer ? 300 : 120;
                const itemHeight = isContainer ? 200 : 120;
                // Simple visible area calculation - use 80% of canvas for safety
                const visibleWidth = canvas.clientWidth * 0.8;
                const visibleHeight = canvas.clientHeight * 0.6; // Keep well above medallion area
                console.log('Canvas dimensions:', canvas.clientWidth, 'x', canvas.clientHeight);
                console.log('Visible area for placement:', visibleWidth, 'x', visibleHeight);
                console.log('Drop coordinates before clamping:', x, y);
                console.log('Item type:', data.type, data.itemType, 'isContainer:', isContainer, 'size:', itemWidth + 'x' + itemHeight);
                // Clamp to visible area with generous margins
                const clampedX = Math.max(20, Math.min(x, visibleWidth - itemWidth));
                const clampedY = Math.max(20, Math.min(y, visibleHeight - itemHeight));
                // ULTRA-SIMPLE positioning for containers - just force them to top-left
                if (isContainer) {
                    console.warn('Container detected - FORCING to top-left area');
                    snapped = { x: 50, y: 50 }; // Always top-left, no exceptions
                    console.log('Container FORCED to position:', snapped.x, snapped.y);
                } else {
                    // Normal positioning for non-containers
                    console.log('Clamped coordinates:', clampedX, clampedY);
                    // Snap to grid for consistent placement (skip for containers during debugging)
                    snapped = this.snapToGrid(clampedX, clampedY);
                    console.log('Final snapped coordinates:', snapped.x, snapped.y);
                }
                if (data.type === 'data-source') {
                    console.log('Adding data source to canvas');
                    this.addDataSourceToCanvas(data, snapped.x, snapped.y);
                } else if (data.type === 'consumption-item') {
                    console.log('Adding consumption item to canvas');
                    this.addConsumptionItemToCanvas(data, snapped.x, snapped.y);
                } else if (data.type === 'canvas-item') {
                    console.log('Adding canvas item to canvas:', data.itemType, 'at coordinates:', snapped.x, snapped.y);
                    this.addCanvasItem(data.itemType, snapped.x, snapped.y);
                } else {
                    console.log('Unknown data type:', data.type); // DEBUG: Log unknown types
                }
            } catch (error) {
    ...
    const itemId = this.ensureElementId(item, 'canvas-item');
        ...
        `;
        // Place at exact coordinates without additional clamping
        console.log(`Positioning ${itemType} at coordinates:`, x, y); // DEBUG: Log exact positioning
        item.style.left = x + 'px';
        item.style.top = y + 'px';
        item.style.visibility = 'visible';
        console.log(`Element positioned with styles: left=${item.style.left}, top=${item.style.top}`); // DEBUG: Confirm styles applied
        this.setupCanvasItemDrag(item);
        this.setupCanvasItemClick(item);
        this.setupNameEditingForItem(item);
        canvas.appendChild(item);
        ...
        if (this.containerTypes.includes(itemType) && !localStorage.getItem('container-tip-shown')) {
            setTimeout(() => {
                this.showNotification('ðŸ’¡ Double-click containers to manage their contents', 'info', 5000);
    ...
    getItemConfig(itemType) {
        const configs = {
            // Analytics & Models
            'notebook': { icon: 'ðŸ““', name: 'Notebook', type: 'Development' },
            'dataset': { icon: 'ðŸ“Š', name: 'Dataset', type: 'Data' },
            'semantic-model': { icon: 'ðŸ§±', name: 'Semantic Model', type: 'Data Modeling' },
            'ml-model': { icon: 'ðŸ¤–', name: 'ML Model', type: 'Machine Learning' },
            'experiment': { icon: 'ðŸ§ª', name: 'Experiment', type: 'Research' },
            // Storage & Processing
            'pipeline': { icon: 'ðŸ”„', name: 'Data Pipeline', type: 'Data Engineering' },
            'dataflow': { icon: 'ðŸŒŠ', name: 'Dataflow', type: 'Data Engineering' },
            'warehouse': { icon: 'ðŸ¬', name: 'Warehouse', type: 'Storage' },
            'lakehouse': { icon: 'ðŸžï¸', name: 'Lakehouse', type: 'Storage' },
            'data-lake': { icon: 'ï¿½ï¸', name: 'Data Lake', type: 'Raw Data Storage' },
            'etl': { icon: 'âš™ï¸', name: 'ETL Process', type: 'Data Engineering' },
            // Data Sources
            'api': { icon: 'ðŸ”Œ', name: 'API', type: 'Data Source' },
            'file-source': { icon: 'ðŸ“', name: 'File Source', type: 'Data Source' },
            'stream': { icon: 'ðŸŒŠ', name: 'Data Stream', type: 'Real-time Data' },
            'web-scrape': { icon: 'ðŸ•·ï¸', name: 'Web Scraping', type: 'Data Source' },
            // Reporting & Visualization
            'report': { icon: 'ðŸ“ˆ', name: 'Report', type: 'Analytics' },
            'dashboard': { icon: 'ðŸ“‹', name: 'Dashboard', type: 'Analytics' },
            'tableau': { icon: 'ðŸ“Š', name: 'Tableau', type: 'Visualization' },
            'qlik': { icon: 'ðŸ“ˆ', name: 'Qlik', type: 'Visualization' },
            // Data Access
            'rest-api': { icon: 'ï¿½', name: 'REST API', type: 'Data Access' },
            'graphql': { icon: 'ðŸ”—', name: 'GraphQL', type: 'Data Access' },
            'odata': { icon: 'ï¿½', name: 'OData', type: 'Data Access' },
            // Medallion Architecture
            'bronze': { icon: 'ðŸ¥‰', name: 'Bronze', type: 'Raw data' },
            'silver': { icon: 'ðŸ¥ˆ', name: 'Silver', type: 'Cleaned data' },
            'gold': { icon: 'ðŸ¥‡', name: 'Gold', type: 'Modelled data' },
            'platinum': { icon: 'ðŸ’Ž', name: 'Platinum', type: 'ML ready data' },
            // Governance & Security
            'data-catalog': { icon: 'ðŸ”', name: 'Data Catalog', type: 'Governance' },
            'data-lineage': { icon: 'ðŸ—ºï¸', name: 'Data Lineage', type: 'Governance' },
            'purview': { icon: 'ðŸ‘ï¸', name: 'Microsoft Purview', type: 'Governance' },
            'onelake-hub': { icon: 'ðŸ¢', name: 'OneLake Data Hub', type: 'Governance' },
            'data-quality': { icon: 'âœ…', name: 'Data Quality', type: 'Quality Management' },
            'dq-rules': { icon: 'ðŸ“‹', name: 'DQ Rules', type: 'Quality Management' },
            'data-profiling': { icon: 'ðŸ“Š', name: 'Data Profiling', type: 'Quality Management' },
            'security-policy': { icon: 'ðŸ›¡ï¸', name: 'Security Policy', type: 'Security' },
            'rls': { icon: 'ðŸ‘¤', name: 'Row Level Security', type: 'Security' },
            'abac': { icon: 'ðŸ”‘', name: 'ABAC', type: 'Security' },
            'pii-classification': { icon: 'ðŸ•µï¸', name: 'PII Classification', type: 'Security' },
            'data-classification': { icon: 'ðŸ·ï¸', name: 'Data Classification', type: 'Security' },
            // Containers & Groups
            'api-collection': { icon: 'ðŸ“¦', name: 'API Collection', type: 'Container' },
            'schema-container': { icon: 'ðŸ“', name: 'Schema', type: 'Container' },
            'table-group': { icon: 'ðŸ“‹', name: 'Table Group', type: 'Container' },
            'process-container': { icon: 'âš™ï¸', name: 'Process Group', type: 'Container' },
            'zone-container': { icon: 'ðŸ—ï¸', name: 'Data Zone', type: 'Container' }
        };
        return configs[itemType] || { icon: 'â“', name: 'Unknown', type: 'Unknown' };
    }
    ...
    initializeTemplates() {
        return {
            medallion: {
                ...
                description: "Bronze â†’ Silver â†’ Gold data lakehouse pattern",
                items: [
        ...
        item.addEventListener('click', (e) => {
            if (this.connectionMode) {
                ...
                if (!localStorage.getItem('properties-tip-shown')) {
                    this.showNotification('ðŸ’¡ Double-click or right-click for Properties', 'info', 4000);
    ...
    lockItem(item) {
        try {
            // DEBUG: Track position before locking
            const beforeLock = { left: item.style.left, top: item.style.top };
            console.log('Locking item - position before:', beforeLock);
            item.dataset.locked = 'true';
            item.classList.add('locked-item');
            // DEBUG: Track position after locking
            const afterLock = { left: item.style.left, top: item.style.top };
            console.log('Locking item - position after:', afterLock);
...
const PDFPageGenerators = {
    ...
    overview: (doc, data, template, branding) => {
        ...
        doc.text(`â€¢ Architecture Complexity: ${complexity} (${data.items.length} components)`, 50, y);
        y += 18;
        // Connection density
        ...
        doc.text(`â€¢ System Integration: ${connectivity} (${(data.connections || []).length} connections)`, 50, y);
        y += 18;
        // Data source diversity
        const types = [...new Set(data.items.map(i => i.type || 'Unknown'))];
        doc.text(`â€¢ Technology Diversity: ${types.length} different component types`, 50, y);
        y += 18;
        ...
        y += 20;
        doc.setFontSize(11);
        doc.text('â€¢ Consolidate data sources to reduce complexity', 50, y);
        y += 15;
        doc.text('â€¢ Implement unified governance framework', 50, y);
        y += 15;
        doc.text('â€¢ Establish data quality monitoring', 50, y);
    },
...
async function exportPDF(options={}) {
    ...
    try {
        ...
        const typeIcons = {
            'Data Lake':'ðŸ—„ï¸','Bronze':'ðŸ¥‰','Silver':'ðŸ¥ˆ','Gold':'ðŸ¥‡','Semantic Model':'ðŸ“Š','Power BI':'ðŸ“ˆ','Excel':'ðŸ“‘','Source':'ðŸ—‚ï¸','ETL':'âš™ï¸','Warehouse':'ðŸ¬','Lakehouse':'ðŸ’§','Dashboard':'ðŸ“Š'
        };
        const pickIcon = (it) => {
...
/ /   F O R C E   C A C H E   R E F R E S H   -   1 0 / 0 1 / 2 0 2 5   0 8 : 5 5 : 0 1 
